...
qplot(x=(realtime_docker/1000)/60, y=(realtime_native/1000)/60, data=NF_stats, main="", color=name, ylab="Native (mins)", xlab="Docker (mins)") + geom_point(size = 3) + coord_cartesian(xlim=c(0,max_axis), ylim=c(0,max_axis)) + geom_abline(intercept = 0)+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14), axis.title.y=element_text(size=16), axis.title.x=element_text(size=16),legend.text=element_text(size=10),legend.title=element_text(size=10)) + labs(color = "Task", title='PIPER - Tasks avg. time')
dev.off()
