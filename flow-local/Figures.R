...
qplot(x=realtime_docker, y=realtime_native, data=NF_stats, main="", color=name, ylab="Native", xlab="Docker") + geom_point(size = 3) + geom_abline(intercept = 1)+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14), axis.title.y=element_text(size=16), axis.title.x=element_text(size=16),legend.text=element_text(size=10),legend.title=element_text(size=10)) + labs(color = "Tasks")
dev.off()
