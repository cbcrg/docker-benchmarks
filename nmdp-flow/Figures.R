...
qplot(x=realtime_docker_log10, y=realtime_native_log10, data=NF_stats, main="", color=name, ylab="Native tasks avg. time (mins)", xlab="Docker tasks avg. time (mins)") + 
    ...
    geom_point( data = NF_median, aes(x=log10((realtime_docker/1000)/60), y=log10((realtime_native/1000)/60), shape = shape, color = NA), size = 3) +
    geom_point( data = NF_mean, aes(x=log10((realtime_docker/1000)/60), y=log10((realtime_native/1000)/60), shape = shape, color = NA), size = 3) +    
